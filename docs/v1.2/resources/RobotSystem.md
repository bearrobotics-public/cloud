...
!!! Note
    When rebooting or shutting down the robot, a response will return immediately to acknowledge the request but may take several minutes before the robot reconnects (reboot) or powers off (shutdown).
...
| ErrorCode  | Description |
|------------|-------------|
| `INVALID_ARGUMENT` | Invalid request: `robot_id` empty, `system_command` missing, reboot with `type: TYPE_UNKNOWN`, or unknown command type. |

...
The list of missions to create, in execution order. Each entry is the same
[`Mission`](#createmission) wrapper used by [CreateMission](#createmission).
Only one [mission type](../../concepts/mission.md#mission-types) may be set per
entry.
...
| ErrorCode  | Description |
|------------|-------------|
| `FAILED_PRECONDITION`   | The robot is already executing another mission. CreateMissionWorkflow is only valid when there is no running mission on the target robot, with its state in one of the [terminal states](#state-enum), i.e., Cancelled, Succeeded, Failed, Default. |
| `INVALID_ARGUMENT`    | The client supplied a request with invalid format. This covers empty `robot_id`, missing or invalid `mission_workflow`, workflow type not set to ServiWorkflow or CartiWorkflow, or workflow type not supported for the robot's family. Client should update their usage to have a correctly formatted request. |
| `PERMISSION_DENIED` | Attempting to create a workflow for a `robot_id` you don't own. |
...
| `INVALID_ARGUMENT`    | The client supplied a request with invalid format. This covers empty `robot_id` and other format errors. Client should update their usage to have a correctly formatted request. |
| `PERMISSION_DENIED` | Attempting to skip goal for a `robot_id` you don't own. |
| `NOT_FOUND` | The specified robot ID does not exist or is not accessible. |
